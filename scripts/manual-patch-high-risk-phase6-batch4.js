...
const helperCode = `
// --- INJECTED MILESTONE BRIDGE (BATCH 4) ---
...
function patchCubeMove() { return applyScoreMilestonePatch('cube-move', 100); }
function patchFasterOrSlower() { return applyScoreMilestonePatch('faster-or-slower', 100); }
function patchQuizGame2() { return applyScoreMilestonePatch('quiz-game-2', 50); }
function patchCarRush() { return applyScoreMilestonePatch('car-rush', 500); }
function patchFillTheWater() { return applyScoreMilestonePatch('fill-the-water', 500); }
function patchChibiHero() { return applyScoreMilestonePatch('chibi-hero', 200); }
function patchTappyDumont() { return applyScoreMilestonePatch('tappy-dumont', 100); }
function patchHitVillains() { return applyScoreMilestonePatch('hit-villains', 200); }
function patchWeaponStrike() { return applyScoreMilestonePatch('weapon-strike', 200); }
function patchThiefChallenge() { return applyScoreMilestonePatch('thief-challenge', 250); }
function patchQuizGames() { return applyScoreMilestonePatch('quiz-games', 50); }
function patchTrueOrFalse() { return applyScoreMilestonePatch('true-or-false', 50); }

const targetPatchers = {
    'cube-move': patchCubeMove,
    'faster-or-slower': patchFasterOrSlower,
    'quiz-game-2': patchQuizGame2,
    'car-rush': patchCarRush,
    'fill-the-water': patchFillTheWater,
    'chibi-hero': patchChibiHero,
    'tappy-dumont': patchTappyDumont,
    'hit-villains': patchHitVillains,
    'weapon-strike': patchWeaponStrike,
    'thief-challenge': patchThiefChallenge,
    'quiz-games': patchQuizGames,
    'true-or-false': patchTrueOrFalse
};
...
for (let i = 0; i < lines.length; i++) {
    ...
    if (isTarget) {
        ...
        if (!success) {
            skippedCount++;
            lines[i] = line.replace(/\| high \|.*?\|/, '| high | Phase 6 Batch 4 skipped |');
            if (!report.includes(`| ${targetSlug} | Yes`)) {
                report += `| ${targetSlug} | No | - | - | - | No | Untested | Exact signature not found |\n`;
            }
        } else {
            patchedCount++;
            lines[i] = line.replace(/\| high \|.*?\|/, '| high | Phase 6 Batch 4 patched |');
        }
    }
}

fs.writeFileSync(mdPath, lines.join('\n'), 'utf8');
fs.writeFileSync(path.join(__dirname, '../LEGACY_REWARD_PHASE6_BATCH4_REPORT.md'), report, 'utf8');

console.log(`Batch 4 Patched: ${patchedCount}`);
console.log(`Batch 4 Skipped: ${skippedCount}`);
