...
const helperCode = `
// --- INJECTED MILESTONE BRIDGE (BATCH 8) ---
...
function patchLipSyncEditor() { return applyScoreMilestonePatch('lip-sync-editor', 500); }
function skipTicTacToe() { return false; }
function skipAntidoteMixer() { return false; }
function skipFaceSwapMemory() { return false; }
function skipDrawPixels() { return false; }
function skipSnakeLadders() { return false; }
function skipLudo() { return false; }
function skipPlayChess() { return false; }
function skipSpiderSolitaire() { return false; }
function skipFourColors() { return false; }
function skipFruitMerge() { return false; }
function skipJoJoRun() { return false; }
function skipFroggyJump() { return false; }

const targetPatchers = {
    'lip-sync-editor': patchLipSyncEditor,
    'tic-tac-toe': skipTicTacToe,
    'antidote-mixer': skipAntidoteMixer,
    'face-swap-memory': skipFaceSwapMemory,
    'draw-pixels': skipDrawPixels,
    'snake-ladders': skipSnakeLadders,
    'ludo': skipLudo,
    'play-chess': skipPlayChess,
    'spider-solitaire': skipSpiderSolitaire,
    'four-colors': skipFourColors,
    'fruit-merge': skipFruitMerge,
    'jo-jo-run': skipJoJoRun,
    'froggy-jump': skipFroggyJump
};

const skipReasons = {
    'tic-tac-toe': 'completion state not detectable',
    'antidote-mixer': 'completion state hidden',
    'face-swap-memory': 'completion state hidden',
    'draw-pixels': 'completion state hidden',
    'snake-ladders': 'completion state hidden',
    'ludo': 'completion state hidden',
    'play-chess': 'completion state hidden',
    'spider-solitaire': 'completion state hidden',
    'four-colors': 'completion state hidden',
    'fruit-merge': 'score hidden inside physics object',
    'jo-jo-run': 'score hidden inside physics object',
    'froggy-jump': 'score hidden inside physics object'
};
...
for (let i = 0; i < lines.length; i++) {
    ...
    if (isTarget) {
        ...
        if (!success) {
            skippedCount++;
            lines[i] = line.replace(/\| (high|low) \|.*?\|/, `| $1 | documented skip | ${skipReasons[targetSlug]} |`);
            if (!report.includes(`| ${targetSlug} | Yes`)) {
                report += `| ${targetSlug} | No | - | - | - | No | Untested | ${skipReasons[targetSlug]} |\n`;
            }
        } else {
            patchedCount++;
            lines[i] = line.replace(/\| (high|low) \|.*?\|/, `| $1 | Phase 6 Batch 8 patched | successfully patched |`);
        }
    }
}

fs.writeFileSync(mdPath, lines.join('\n'), 'utf8');
fs.writeFileSync(path.join(__dirname, '../LEGACY_REWARD_PHASE6_BATCH8_FINAL_REPORT.md'), report, 'utf8');

console.log(`Batch 8 Patched: ${patchedCount}`);
console.log(`Batch 8 Documented Skip: ${skippedCount}`);
