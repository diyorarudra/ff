...
function patch() {
    const file = 'games/antidote-mixer/index.html';
    let content = fs.readFileSync(file, 'utf8');
    
    // Inject helper
    content = content.replace('const canvas = document.getElementById(\'gameCanvas_40\');', helper + '\n    const canvas = document.getElementById(\'gameCanvas_40\');');
    ...
    const target = 'if(checkWin()) {\n            isAnimating = true;';
    const replacement = 'if(checkWin()) {\n            if (!window.ffRoundRewardGiven) {\n                window.ffRoundRewardGiven = true;\n                ffTriggerRewardEvent("LEVEL_COMPLETE", { level: level });\n            }\n            isAnimating = true;';
    content = content.replace(target, replacement);
    
    fs.writeFileSync(file, content, 'utf8');
    console.log('patched antidote-mixer');
}
