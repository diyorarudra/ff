...
function generateLevel(lvl) {
    ...
    // Create an array of layers (4 of each color)
    let layers = [];
    for(let c=1; c<=numColors; c++) {
        for(let i=0; i<TUBE_CAPACITY; i++) layers.push(c);
    }
...
function renderTubes() {
    ...
    tubes.forEach((tData, idx) => {
        ...
        tData.forEach(colorVal => {
            ...
            lEl.className = `water-layer color-${colorVal}`;
            tEl.appendChild(lEl);
        });
