...
const instructions = {
    ...
    55: "Click and drag to set power and angle, release to shoot.",
    ...
    60: "Click and drag to position pieces on the board.",
    ...
    69: "Use Left/Right arrows to steer your car along the road.",
    71: "Click inside the canvas to drop items and merge matching types.",
    79: "Click the correct answer to progress through the quiz."
};
...
const microEngines = {
    51: `
        ...
        function loop() {
            ...
            for(let i=pipes.length-1; i>=0; i--) {
                ...
                if(p.x < 100 && !p.passed) { score++; p.passed = true; document.getElementById('score').innerText=score; }
    ...
    53: `
        ...
        function loop() {
            ...
            for(let i=ob.length-1;i>=0;i--){ ob[i].y+=5; ctx.fillStyle='#000'; ctx.fillRect(ob[i].x,ob[i].y,30,30); if(ob[i].y>c.height){ob.splice(i,1); score++; document.getElementById('score').innerText=score;} }
            ctx.fillStyle='#fff'; ctx.font='18px sans-serif'; ctx.fillText('Score: '+score, 20,30); requestAnimationFrame(loop);
        }
    ...
    54: `
        ...
        function loop() {
            ...
            for(let i=aliens.length-1;i>=0;i--){ 
                ...
                for(let j=lasers.length-1;j>=0;j--){ if(Math.hypot(lasers[j].x-aliens[i].x, lasers[j].y-aliens[i].y)<20) { aliens.splice(i,1); lasers.splice(j,1); score+=10; document.getElementById('score').innerText=score; break; } }
            }
            ctx.fillStyle='#fff'; ctx.font='18px sans-serif'; ctx.fillText('Score: '+score, 20,30); requestAnimationFrame(loop);
        }
    ...
    55: `
        ...
        function run() {
            ...
            if(active) { vy += 0.35; bx += vx; by += vy; if(bx > 310 && bx < 380 && by > 140 && by < 170 && vy > 0) { score++; document.getElementById('score').innerText=score; active = false; bx = 150; by = 300; } if(by > 450) { active = false; bx = 150; by = 300; } }
    ...
    56: `
        ...
        canvas.addEventListener('click', (e) => { const r=canvas.getBoundingClientRect(); const scY=canvas.height/r.height; let y=(e.clientY-r.top)*scY; for(let i=0; i<4; i++) { if(y>=120+i*60 && y<=160+i*60 && i===2) { completed=true; render(); document.getElementById('score').innerText=100; } } });
        render();
    `,
    57: `
        ...
        function draw(){
            ...
            ctx.fillStyle='#fff'; ctx.font='18px sans-serif'; ctx.fillText('Roll Dice (Click)', 150,20);
        }
    ...
    58: `
        ...
        function draw(){
            ...
            ctx.fillStyle='#000'; ctx.font='18px sans-serif'; ctx.fillText('Click to progress Ludo positions: '+p.join(','), 60, 200);
        }
    ...
    65: `
        ...
        c.addEventListener('click',(e)=>{ const r=c.getBoundingClientRect(); const x=e.clientX-r.left; let idx=Math.floor((x-50)/80); if(idx>=0 && idx<hand.length){ center=hand[idx]; hand.splice(idx,1); hand.push(Math.floor(Math.random()*4)); score++; document.getElementById('score').innerText=score; draw(); } }); draw();
    `,
    67: `
    ...
    68: `
        ...
        function draw(){ ctx.fillStyle='#131a26'; ctx.fillRect(0,0,c.width,c.height); ctx.fillStyle='#fff'; ctx.font='18px sans-serif'; ctx.fillText('Click to play audio snippet.', 50,200); }
    ...
    69: `
        ...
        let pos=0, curve=0, playerX=200, score=0;
        function loop(){
            pos+=5; score++; document.getElementById('score').innerText=Math.floor(score/10); curve=Math.sin(pos*0.01)*100;
            ...
            ctx.fillStyle='#ef4444'; ctx.fillRect(playerX-20,320,40,30);
    ...
    71: `
        ...
        function step() {
            ...
            for(let i=0; i<list.length; i++) { for(let j=i+1; j<list.length; j++) { if(Math.hypot(list[i].x-list[j].x, list[i].y-list[j].y) < 40) { list.splice(j,1); list[i].r=35; score+=10; document.getElementById('score').innerText=score; } } }
...
function generateMicroEngine(id, title) {
    ...
    if (engineType === 0) {
        ...
        function loop(){
            ...
            for(let i=p.length-1;i>=0;i--){ p[i].vy+=0.5; p[i].y+=p[i].vy; ctx.beginPath(); ctx.arc(p[i].x,p[i].y,6,0,Math.PI*2); ctx.fill(); if(p[i].y>c.height){ p.splice(i,1); score++; document.getElementById('score').innerText=score; } }
        ...
        return `
        const c=document.getElementById('gameCanvas_${id}'); const ctx=c.getContext('2d');
        let px=50, py=350, vy=0, plat=[{x:0,w:400,y:380},{x:200,w:100,y:300},{x:350,w:80,y:230}], score=0;
        function loop(){
            ctx.fillStyle='#131a26'; ctx.fillRect(0,0,c.width,c.height);
            ...
            if(py>c.height) { py=0; px=50; score=0; }
        ...
        window.addEventListener('keydown',(e)=>{if(e.key==='ArrowRight'){px+=5; score++;} if(e.key==='ArrowLeft')px-=5; if(e.key===' '&&vy===0)vy=-12; document.getElementById('score').innerText=score;}); loop();`;
        ...
        c.addEventListener('click',(e)=>{ const r=c.getBoundingClientRect(); const idx=Math.floor(((e.clientX-r.left)/(r.width/4))) + Math.floor(((e.clientY-r.top)/(r.height/4)))*4; if(grid[idx]>0){grid[idx]=0;s+=10; document.getElementById('score').innerText=s;} }); loop();`;
    } else {
        return `
        const c=document.getElementById('gameCanvas_${id}'); const ctx=c.getContext('2d');
        let b=[]; let s=0;
...
for (let i = 51; i <= 100; i++) {
    ...
    if (fs.existsSync(fileLoc)) {
        ...
        // Fix the instruction tagline
        const brokenTagline = 'Swipe or use arrows to combine the <span class="text-cyan-400 font-bold">same numbers</span>.';
        const plainBrokenTagline = "Swipe or use arrows to combine the same numbers.";
        
        let newTagline = getInstructionText(i, titles[i]);
        
        if (text.includes(brokenTagline)) {
            text = text.replace(brokenTagline, newTagline);
        } else if (text.includes(plainBrokenTagline)) {
            text = text.replace(plainBrokenTagline, newTagline);
        }
        ...
        if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
            ...
            modifiedCount++;
            console.log(`[Antigravity Automation]: Deployed active mini-engine and fixed tagline for ${titles[i]} at games/game${i}/index.html`);
        }
    }
}
