...
const specificEngines = {
    ...
    67: `
        ...
        canvas.addEventListener('mousedown', (e) => { const r=canvas.getBoundingClientRect(); const scX=canvas.width/r.width; let x=(e.clientX-r.left)*scX; for(let i=0; i<7; i++) { if(x>=100+i*85 && x<=175+i*85) { play(261.63 + i*30); ctx.fillStyle='#ddd'; ctx.fillRect(100+i*85, 100, 75, 300); setTimeout(layout, 100); } } });
        layout();
    `,
    68: `
...
function assignEngine(id, title) {
    if (specificEngines[id]) return specificEngines[id];
    
    const t = title.toLowerCase();
    if (t.includes('run') || t.includes('hero') || t.includes('car') || t.includes('escape') || t.includes('rush')) return engines.runner(id, title, '#22c55e', '#ef4444');
    if (t.includes('shoot') || t.includes('space') || t.includes('strike') || t.includes('cannon') || t.includes('villain')) return engines.shooter(id, title, '#38bdf8', '#ef4444');
    if (t.includes('quiz') || t.includes('math') || t.includes('word') || t.includes('true') || t.includes('guess')) return engines.quiz(id, title);
    if (t.includes('puzzle') || t.includes('chess') || t.includes('card') || t.includes('match') || t.includes('tic')) return engines.puzzle(id, title);
    
    return engines.clicker(id, title, '#facc15', '#a855f7');
}

function getInstruction(id, title) {
    if (specificEngines[id]) {
        if (id === 51) return "Click or tap to flap and avoid the obstacles.";
        if (id === 52) return "Click and drag on the grid to draw pixels.";
        if (id === 53) return "Use A/D and Left/Right arrows to move both blocks.";
        if (id === 54) return "Use Left/Right arrows to move and Space to shoot.";
        if (id === 55) return "Click and drag to aim, release to shoot toward the hoop.";
        if (id === 56) return "Click the correct answer to progress through the quiz.";
        if (id === 57) return "Click to roll the dice and move your piece.";
        if (id === 58) return "Click to progress your pieces along the Ludo board.";
        if (id === 60) return "Click and drag to position pieces on the board sandbox.";
        if (id === 64) return "Click to distribute cards and build descending suits.";
        if (id === 65) return "Click the deck to play a matching color card.";
        if (id === 67) return "Click the piano keys to play a musical scale.";
        if (id === 68) return "Click to listen to the audio snippet and guess.";
        if (id === 69) return "Use Left and Right keys to steer along the curve.";
        if (id === 71) return "Click inside the canvas viewport to drop items and merge identical tiers.";
        if (id === 79) return "Click the correct answer boxes to score points.";
    }

    const t = title.toLowerCase();
    if (t.includes('run') || t.includes('hero') || t.includes('car') || t.includes('escape') || t.includes('rush')) return "Use Space or tap to jump and avoid obstacles.";
    if (t.includes('shoot') || t.includes('space') || t.includes('strike') || t.includes('cannon') || t.includes('villain')) return "Use Left/Right arrows to move, Space to shoot.";
    if (t.includes('quiz') || t.includes('math') || t.includes('word') || t.includes('true') || t.includes('guess')) return "Click or tap the correct option to answer.";
    if (t.includes('puzzle') || t.includes('chess') || t.includes('card') || t.includes('match') || t.includes('tic')) return "Click or tap to interact with the board.";
    
    return "Click or tap the targets before they disappear.";
}
...
for (let i = 51; i <= 100; i++) {
    ...
    if (fs.existsSync(fileLoc)) {
        ...
        // Fix the \n strings in HTML
        // Note: the previous scripts literally inserted '\n' strings into the text
        text = text.replace(/\\n/g, '');
        // Fix the stray 'n' that might have been left
        ...
        if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
            const enginePayload = assignEngine(i, title);
...
console.log('Restored specific logic mapping for ' + count + ' games.');
